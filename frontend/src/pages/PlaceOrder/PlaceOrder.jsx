...
const PlaceOrder = () => {
  ...
  const placeOrder = async (event) => {
    ...
    let orderData = {
      address: data,
      items: orderItems,
      amount: getTotalCartAmount() + 2, // Total amount including delivery charge
    };
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const options = {
          key: "rzp_test_73PruWjLurCwKg", // Razorpay test key
          amount: amount,
          currency: currency,
          name: "Your Company",
          ...
          handler: async function (response) {
            ...
            // ✅ Send correct order ID for verification
            window.location.replace(
              `http://localhost:5173/verify?success=true&razorpay_order_id=${razorpay_order_id}&razorpay_payment_id=${razorpay_payment_id}&razorpay_signature=${razorpay_signature}`
            );
          },
          prefill: {
